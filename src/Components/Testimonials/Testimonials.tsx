...
export default function Testimonials() {
  ...
  const testimonials = [
    {
      name: "Daniel Ortega", role: t("testimonials.role1"), company: "Stripe",
    ...
    <section className="grad-navy relative overflow-hidden py-20 sm:py-28">
      ...
      <div className="mx-auto max-w-[1400px] px-4 sm:px-6">
        <Reveal className="mx-auto max-w-xl text-center">
          <p className="mb-2 text-sm font-semibold text-blue-300">{t("testimonials.tag")}</p>
          ...
          {testimonials.map((item, i) => (
            <Reveal key={item.name} delay={i * 100}>
              <div className="flex h-full flex-col rounded-2xl glass-dark p-6 shadow-soft-dark">
                ...
                <p className="mt-4 flex-1 text-[15px] leading-relaxed text-slate-200">"{item.text}"</p>
                <div className="mt-6 flex items-center gap-3 border-t border-white/10 pt-5">
                  <img src={item.image} alt={item.name} className="h-11 w-11 rounded-full object-cover ring-2 ring-white/20" />
                  <div className="min-w-0">
                    <p className="font-display text-[15px] font-bold text-white">{item.name}</p>
                    <p className="truncate text-[12.5px] text-slate-400">{item.role} · {item.company}</p>
                  </div>
